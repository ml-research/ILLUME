# Generate embeddings for ILLUME training
# Note that embeddings from previous iteration will automatically be overwritten
...
if __name__ == '__main__':
    ...
    parser.add_argument('--iteration', '-i', dest='iteration', type=int, help='upcoming training iteration')
    ...
    df_tmp = pd.read_json(f'/workspace/repositories/ILLUME/results/vqax/it{args.iteration}/train_samples.json')
    df_tmp = df_tmp.loc[df_tmp.st]
    index = splits[0]
    max_split = splits[1]
    ...
    dest_dir = '/workspace/datasets/COCO/VQA-X/train_ILLUME_tmp'
    os.makedirs(dest_dir, exist_ok=True)
    img_prefix = 'COCO_train2014_'

    rtpt = RTPT(name_initials='TBD', experiment_name=f'VQAX_PrepILLUME_{args.iteration}', max_iterations=len(df))

    model, transforms, tokenizer = get_multimodal_model(config_path, ckpt_path=ckpt_path, model_dir=model_dir, lm_ckpt_path=None)
    model.half()
    model.eval()
    model.to(device)
    rtpt.start()
    ...
    for idx, item in tqdm(df.iterrows(), total=len(df)):
        ...
        answer = item["answer"]
        explanation = item[f"explanation_sample"]
        ...
        with torch.no_grad():
            ...
            path_question_input = os.path.join(dest_dir, f'{sample_id:012d}_question_input.pt')
            path_explanation_input = os.path.join(dest_dir, f'{sample_id:012d}_explanation_input.pt')

            path_question_label = os.path.join(dest_dir, f'{sample_id:012d}_question_label.pt')
            path_explanation_label = os.path.join(dest_dir, f'{sample_id:012d}_explanation_label.pt')
