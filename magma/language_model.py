...
from .gpt_neo_ours import GPTNeoForCausalLMOurs
import gdown
import tarfile
...
def get_language_model(
    ...
    if "gpt-neo" in name:
        model = GPTNeoForCausalLMOurs.from_pretrained(name, config=config)
    elif name == "gptj":
        if from_pretrained:
            download_gptj(model_dir)
            ckpt_path = Path(model_dir) / "j6b_ckpt"
            model = GPTNeoForCausalLMOurs.from_pretrained(
            ...
            if no_init:
                ...
                with no_init_weights():
                    model = GPTNeoForCausalLMOurs(config=config)
            else:
